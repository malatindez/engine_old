...
class Texture {
 public:
  enum class Type {
     ...
     * standard. This was originally introduced around 2012. Support for this
     * is in game engines like Godot, Unreal or Unity3D. Modelling packages
     * which use this are very common now.
     */
    BASE_COLOR,
    NORMAL_CAMERA,
    EMISSION_COLOR,
  ...
  Texture(unsigned char const* data, int width, int height, int channels,
      ...
      : values_(values), type_(type), path_(path) {
    glGenTextures(1, &id_);
  ...
  [[nodiscard]] std::vector<glTexParameterValues> values() const noexcept {
    return values_;
  }
