...
GLint ShaderProgram::checkCompileErrors(GLuint shader, std::string type) {
    ...
    else
    {
        ...
        if (!success) {
            ...
            std::cerr << "ERROR. Shader program linking error of type: " << type << "\n" << infoLog << "\n -- --------------------------------------------------- -- " << std::endl;
#endif
        }
    }
    return success;
...
ShaderProgram::ShaderProgram(std::string vertexShaderCode, std::string fragmentShaderCode, std::string geometryShaderCode) {
...
void ShaderProgram::setVec1(const std::string& name, const glm::vec1& value) const {
...
void ShaderProgram::setMat2x2(const std::string& name, const glm::mat2x2& value) const {
    setMat2(name, value);
}

void ShaderProgram::setMat2x3(const std::string& name, const glm::mat2x3& value) const {
    glUniformMatrix2x3fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &value[0][0]);
}

void ShaderProgram::setMat2x4(const std::string& name, const glm::mat2x4& value) const {
    glUniformMatrix2x4fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &value[0][0]);
}



void ShaderProgram::setMat3x2(const std::string& name, const glm::mat3x2& value) const {
    glUniformMatrix3x2fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &value[0][0]);
}

void ShaderProgram::setMat3x3(const std::string& name, const glm::mat3x3& value) const {
    setMat3(name, value);
}

void ShaderProgram::setMat3x4(const std::string& name, const glm::mat3x4& value) const {
    glUniformMatrix2x4fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &value[0][0]);
}



void ShaderProgram::setMat4x2(const std::string& name, const glm::mat4x2& value) const {
    glUniformMatrix4x2fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &value[0][0]);
}

void ShaderProgram::setMat4x3(const std::string& name, const glm::mat4x3& value) const {
    glUniformMatrix4x2fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &value[0][0]);
}

void ShaderProgram::setMat4x4(const std::string& name, const glm::mat4x4& value) const {
    glUniformMatrix4fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &value[0][0]);
}